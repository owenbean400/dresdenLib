...
Vue.component("Navbar", {
  template:
    '<nav id="navbar" class="bg-dark" v-bind:class="phoneClass"><a class="navlogo" :href="beginningDirectory(\'index.html\')">{{logoName}}</a><div class="link-container" v-bind:class="toggleShowPhoneNav"><div class="phoneNavbar" v-bind:class="{none: !isPhone}"><a class="navlogo" :href="beginningDirectory(\'index.html\')">{{logoName}}</a><svg v-on:click="hideLinks" class="x" version="1.1"id="Layer_1"xmlns="http://www.w3.org/2000/svg"xmlns:xlink="http://www.w3.org/1999/xlink"x="0px"y="0px"viewBox="0 0 64 64"style="enable-background:new 0 0 64 64;" xml:space="preserve"><style type="text/css">.st0 {fill: #341C1C;}</style><polygon class="st0" points="58.9,50.4 50.4,58.9 28.5,35.5 5.1,13.6 13.6,5.1 35.5,28.5 " /><polygon class="st0" points="5.1,50.4 13.6,58.9 35.5,35.5 58.9,13.6 50.4,5.1 28.5,28.5 "/></svg></div><div class="links"><ol><li v-for="(link, name) in links"><a v-bind:href="beginningDirectory(link)">{{name}}</a></li></ol></div></div><div v-on:click="showLinks" class="hamburger"><div v-for="i in 3"></div></div></nav>',
  data: function () {
    return {
      links: links,
