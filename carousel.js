Vue.component("Carousel", {
  ...
  <div class="carsousel">
    <h4>{{title}}</h4> 
    <div class="carsousel-container">
        <svg class="arrow" width="32px" v-on:click="leftMove" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
          viewBox="0 0 64 64" style="enable-background:new 0 0 64 64;" xml:space="preserve">
        <polygon class="st0" points="64,0 0,32 64,64 "/>
        </svg>
        <div class="book" v-for="(data, num) in showingISBN">
            <div class="image-container">
              <img :key="num" :src="data.src" :alt="'book isbn: ' + num" width="120px" height="192px"/>
            </div>
            <p>{{data.title}}</p>
        </div>
        <svg class="arrow" width="32px" v-on:click="rightMove" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
          viewBox="0 0 64 64" style="enable-background:new 0 0 64 64;" xml:space="preserve">
          <polygon class="st0" points="0,0 64,32 0,64 "/>
        </svg>
    </div>
  <div>
    `,
  data: function () {
    let ISBNnumbers = {};
    for (let num of this.isbns) {
      ISBNnumbers[num.toString()] = {
        src: "blank",
        alt: num.toString(),
        title: "loading title",
